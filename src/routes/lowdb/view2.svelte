<script lang="ts">
	...
	function updateSearchNodeArray(node: node) {
		return Object.keys(node).map((id) => {
			return { id, ...node[id] };
		});
	}
	...
	const edgeArray = Object.keys(edge).map((id) => {
		return { id, ...edge[id] };
	});
	...
	function search(e: Event) {
		...
		let userInputText = inputElement.value;
		// 로직
		let lang = langSelector(userInputText);
		autoCompleteVisiblity(inputElement);
		searchFiltered();
		...
		function searchFiltered() {
			switch (lang) {
				case 'kr':
					let LangIndex = 0;
					autoCompleteNodeArr = nodeArray.filter((node) =>
						node.name[langIndex].includes(userInputText)
					);
					break;
				default:
		...
		function autoCompleteVisiblity(input: HTMLInputElement) {
			const autoComplete = input.nextElementSibling;
			if (!autoComplete)
				throw new Error('autoComplete div를 찾을 수 없습니다. 관리자에게 문의하세요');
			if (input.value.length > 0) {
				autoComplete.classList.remove('invisible');
			} else {
				autoComplete.classList.add('invisible');
			}
		}
		function langSelector(userInputText: string) {
			// TODO 영어권 언어는 이걸로 안됨. 사용자한테 언어 선택 스테이터스 받아서 그걸로 처리하는 로직으로 변경해야함
	...
	<div class="flex ml-auto gap-2">
		...
		{#each nodeTypes as type, idx}
			<label class="flex items-center">
				<input
					type="radio"
					bind:group={voidInfo.type}
					name="type"
					class="radio radio-sm peer hidden"
					value={type}
				/>
...
{#each Object.keys(node) as item (item)}
	<div class="flex">
		...
		<button class="btn btn-xs btn-ghost text-slate-400" data-id={item} on:click={deleteNode}
			>d</button
		>
	</div>
{/each}
...
<div class="flex flex-col">
	<input
		type="text"
		class="input input-xs w-screen bg-slate-50"
		on:keyup={search}
		placeholder="search nodename"
	/>
	<div class="relative invisible">
		<div class="absolute left-0 w-full bg-slate-200 text-slate-400 rounded-md opacity-90">
			{#each autoCompleteNodeArr as node}
				<div class=" hover:btn-secondary">
					{JSON.stringify(node)}
				</div>
			{/each}
		</div>
